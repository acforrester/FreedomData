...
```{r efw}
head(efwpnl %>% arrange(-year, iso_code), n = 10)
```

# A Quick Descriptive Analysis

Here I will show the basic usage of the EFW data and a brief analysis. Here I 
will use the `tidyverse` and `plotly` packages to parse and analyze the data.
...
p <- efwpnl %>% 
  ...
  summarize(efw_index = mean(efw_index, na.rm = T)) %>% 
  ggplot(data = .,
         aes(x = region_name,
             y = efw_index))+
...
p <- efwpnl %>% 
  ...
  select(country_name, year, efw_present) %>% 
  ggplot(data = .,
         aes(x  = factor(year),
             y = country_name))+
  geom_raster(aes(fill = efw_present)) +
